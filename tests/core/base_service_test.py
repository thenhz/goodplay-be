...
class BaseServiceTest(TestBase):
    ...
    def setup_method(self, method):
        ...
        self.log_test_info(f"Service test setup complete: {self.service_class.__name__ if self.service_class else 'Unknown'}")
    ...
    def _setup_service_with_mocks(self):
        ...
        if not self.service_class:
            self.log_test_debug("No service_class specified, skipping service setup")
            return
        ...
        self.log_test_debug(f"Service setup complete with {len(self.repository_mocks)} repository mocks and {len(self.external_mocks)} external mocks")
    ...
    def _create_repository_mock(self, repo_name: str) -> MagicMock:
        ...
        self.log_test_debug(f"Created repository mock: {repo_name}")
        return repo_mock
    ...
    def _log_service_performance(self):
        ...
        if hasattr(self, 'service_operation_timings') and self.service_operation_timings:
            self.log_test_debug("Service operation timings:", self.service_operation_timings)
    ...
    def setup_repository_mock_response(self, repo_name: str, method_name: str, return_value: Any):
        ...
        self.log_test_debug(f"Setup {repo_name}.{method_name} to return: {return_value}")
    ...
    def assert_service_success(self, success: bool, message: str, result: Any, expected_message: str = None):
        ...
        self.log_test_debug(f"Service success assertion passed: {message}")
    ...
    def assert_service_failure(self, success: bool, message: str, result: Any, expected_message: str = None):
        ...
        self.log_test_debug(f"Service failure assertion passed: {message}")
    ...
    def assert_service_performance(self, operation_func: Callable, max_time_ms: float = None, *args, **kwargs):
        ...
        self.log_test_debug(f"Service performance assertion passed: {operation_name} took {execution_time_ms:.1f}ms")
        return result
    ...
    def validate_service_response_pattern(self, response: tuple):
        ...
        self.log_test_debug("Service response pattern validation passed")
    ...
    def validate_service_error_handling(self, service_method: Callable, *args, **kwargs):
        ...
        try:
            ...
            if success:
                self.log_test_debug("Service method completed successfully without raising exception")
            else:
                self.log_test_debug(f"Service method returned failure gracefully: {message}")

        except Exception as e:
            # If service method raises exception, it should be logged and handled
            self.log_test_debug(f"Service method raised exception: {str(e)}")
    ...
    def assert_smart_fixture_performance(self):
        ...
        self.log_test_debug(f"Smart Fixture performance check passed - Cache: {cache_hit_ratio:.1%}, Memory: {memory_usage_mb:.1f}MB")
