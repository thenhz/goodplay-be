...
class TestPreferencesServiceGOO35(BasePreferencesTest):
    ...
    def test_get_preferences_with_defaults(self):
        ...
        del user_data['preferences']  # Remove preferences
        self.mock_preferences_repository.get_user_preferences.return_value = user_data
        ...
        self.assert_default_preferences_structure(default_prefs)
    ...
    def test_reset_preferences_to_default(self):
        ...
        self.assert_default_preferences_structure(reset_prefs)
    ...
    def test_get_preferences_by_category_success(self):
        ...
        success, message, result = self.service.get_user_preferences_by_category(
        ...
        assert "notifications" not in result  # Only gaming category
    ...
    def test_update_notification_preferences(self):
        ...
        success, message, result = self.service.update_notification_preferences(
            user_data['_id'], notification_settings
        )

        assert success is True
        assert message == "NOTIFICATION_PREFERENCES_UPDATED"

        # Verify correct notification preferences were updated
        call_args = self.mock_preferences_repository.update_user_preferences.call_args[0]
        updated_prefs = call_args[1]
        notifications = updated_prefs['notifications']
        assert notifications['email_notifications'] is False
        assert notifications['push_notifications'] is True
    ...
    def test_update_privacy_preferences(self):
        ...
        success, message, result = self.service.update_privacy_preferences(
            user_data['_id'], privacy_settings
        )

        assert success is True
        assert message == "PRIVACY_PREFERENCES_UPDATED"

        # Verify privacy settings
        call_args = self.mock_preferences_repository.update_user_preferences.call_args[0]
        updated_prefs = call_args[1]
        privacy = updated_prefs['privacy']
        assert privacy['profile_visibility'] == 'friends_only'
        assert privacy['show_online_status'] is False

    def test_sync_preferences_success(self):
        """Test successful preferences synchronization"""
        user_data = self.create_test_user_with_preferences()
        self.mock_preferences_repository.get_user_preferences.return_value = user_data
        self.mock_preferences_repository.update_user_preferences.return_value = True

        # Mock synchronization scenario
        self.mock_preference_synchronization()

        # Simulate device preferences sync
        device_prefs = self.create_test_preferences(
            theme='light',
            sync_version=2  # Newer version
        )

        success, message, result = self.service.sync_user_preferences(
            user_data['_id'], device_prefs
        )

        assert success is True
        assert message == "PREFERENCES_SYNCED_SUCCESS"
        assert result is not None
        assert result.get('sync_conflicts_resolved', 0) >= 0

    def test_sync_preferences_conflicts(self):
        """Test preferences synchronization with conflicts"""
        user_data = self.create_test_user_with_preferences()
        user_data['preferences']['sync_version'] = 2  # Same version - conflict
        self.mock_preferences_repository.get_user_preferences.return_value = user_data

        device_prefs = self.create_test_preferences(
            theme='dark',
            sync_version=2  # Same version - conflict
        )
        ...
        assert success is True
        assert message == "PREFERENCES_SYNCED_WITH_CONFLICTS"
        assert result is not None
    ...
    def test_export_preferences(self):
        ...
        success, message, result = self.service.export_user_preferences(user_data['_id'])

        assert success is True
        assert message == "PREFERENCES_EXPORTED_SUCCESS"
        assert result is not None
    ...
    def test_import_preferences(self):
        ...
        import_data = {
            ...
            "export_timestamp": "2023-12-01T10:00:00Z",
            "version": "1.0"
        }

        success, message, result = self.service.import_user_preferences(
            user_data['_id'], import_data
        )

        assert success is True
        assert message == "PREFERENCES_IMPORTED_SUCCESS"

    def test_validate_preferences_schema(self):
        """Test preferences schema validation"""
        # Valid preferences
        valid_prefs = self.create_test_preferences()
        is_valid, errors = self.service.validate_preferences_schema(valid_prefs)
        ...
        invalid_prefs = {
            ...
            "notifications": "not_a_dict"  # Should be dict
        }
        is_valid, errors = self.service.validate_preferences_schema(invalid_prefs)
        assert is_valid is False
        assert len(errors) > 0

    def test_get_preferences_history(self):
        """Test retrieving preferences change history"""
        user_data = self.create_test_user_with_preferences()
        self.mock_preferences_repository.get_user_preferences.return_value = user_data
        self.mock_preferences_repository.get_preferences_history.return_value = [
            {
        ...
        success, message, result = self.service.get_preferences_history(user_data['_id'])

        assert success is True
        assert message == "PREFERENCES_HISTORY_RETRIEVED"
        assert result is not None
